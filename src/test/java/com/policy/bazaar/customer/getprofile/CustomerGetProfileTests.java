package com.policy.bazaar.customer.getprofile;
...
public class CustomerGetProfileTests {
	...
	public void getprofileFound() {
		try {
			...
			mvc.perform(MockMvcRequestBuilders.get("/customers/getProfile/2015").header("Authorization", values)
					.accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
					.andExpect(jsonPath("$.data.firstname").value("Lucky"))
					.andExpect(jsonPath("$.data.lastname").value("Kumar"))
					.andExpect(jsonPath("$.data.email").value("l@k.com"))
	...
	public void getprofileNotFound() {
		try {
			...
			ResultActions andExpect = mvc.perform(MockMvcRequestBuilders.get("/customers/getProfile/2090").header("Authorization", values)
					.accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
